...
class _PlansPageState extends ConsumerState<PlansPage> {
  ...
  double _calcularTotalEstimado() {
    double total = 0;
    _productosSeleccionados.forEach((id, cantidad) {
      final producto = _productos.firstWhere((p) => p.id == id, orElse: () => const Producto(id: -1, nombre: ''));
      if (producto.id != -1 && producto.precio != null) {
  ...
  Future<void> _crearCotizacion() async {
    ...
    try {
      ...
      final cotizacion = Cotizacion(
        ...
        totalAmount: _calcularTotalEstimado(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Cotización de Planes'),
        elevation: 0,
      ),
      body: Form(
        key: _formKey,
  ...
  Widget _buildTotalSection() {
    final total = _calcularTotalEstimado();
    return Card(
